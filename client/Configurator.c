...
static void authorizedPasswords(List* list, struct Context* ctx)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        // <tigu>
        int64_t* upLimitKbps = Dict_getInt(d, String_CONST("max_speed_up"));
        int64_t* downLimitKbps = Dict_getInt(d, String_CONST("max_speed_down"));

        if (!upLimitKbps) {
            upLimitKbps = Allocator_malloc(child,sizeof(int64_t));
            *upLimitKbps = 0;
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (connectTo) {
            ...
            while (entry != NULL) {
                ...
                // start -- <tiguwita>
                int64_t* upLimitKbps = Dict_getInt(all, String_CONST("max_speed_up"));
                int64_t* downLimitKbps = Dict_getInt(all, String_CONST("max_speed_down"));
                ...
                if (!downLimitKbps) {
                    ...
                    *downLimitKbps = 0;
                    ...
                    Log_info(ctx->logger, "Download speed limitation for peer [%s] "
                             "set at [%ukb/s].", pub_d->bytes, (unsigned)*downLimitKbps);
                } else {
